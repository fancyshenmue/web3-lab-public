...
const CopyableFieldDark = ({ label, value, isMono = false, onContextMenu }: { label: string, value: string | boolean, isMono?: boolean, onContextMenu?: (e: React.MouseEvent, val: string) => void }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-sm text-gray-300 break-all flex-1 ${isMono ? 'font-mono text-blue-300' : ''}`}>
        {stringValue}
      </span>
      <span className={`text-xs font-medium transition-opacity flex-shrink-0 ${copied ? 'text-green-400 opacity-100' : 'text-blue-400 opacity-0 group-hover:opacity-100'}`}>
        {copied ? '✓' : 'Copy'}
      </span>
    </div>
  )
      ...
      <div className="border-b border-gray-800">
        <button
          ...
          className={`w-full flex items-center justify-between px-6 py-4 text-sm font-semibold transition-colors
            ${isExpanded ? 'text-white bg-blue-600/10 border-l-2 border-blue-500' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200 border-l-2 border-transparent'}
          `}
        >
          ...
          {hasSubmenu ? (
            <svg className={`w-4 h-4 transition-transform ${isExpanded ? 'rotate-180 text-blue-400' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
          ) : (
            isExpanded && <span className="w-1.5 h-1.5 rounded-full bg-blue-400"></span>
          )}
        </button>
            ...
            {(['ERC20', 'ERC721', 'ERC1155'] as TokenType[]).map(t => (
              <button
                ...
                className={`w-full text-left pl-14 pr-4 py-2 text-[13px] transition-all
                  ${activeToken === t
                    ? 'text-white font-medium bg-blue-600/20 shadow-[inset_2px_0_0_0_#3b82f6]'
    ...
    <div className="space-y-8">
      ...
      <div className={`rounded-2xl p-6 border backdrop-blur-md shadow-2xl ${
        isWalletUser
          ...
          : 'bg-gradient-to-br from-blue-900/30 to-fuchsia-900/30 border-blue-500/20'
      }`}>
        <div className="flex items-center gap-4">
          {userInfo?.picture ? (
            ...
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-fuchsia-600 flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-blue-500/20">
            ...
            {isWalletUser ? (
              <>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <p className="text-sm font-mono text-blue-300">{truncateAddress(walletAddress!)}</p>
                  <button
                    onClick={() => copyAddress()}
        ...
        <div className="p-4 space-y-3">
          {identitiesLoading ? (
            <div className="text-center py-6">
              <div className="animate-spin inline-block rounded-full h-6 w-6 border-2 border-gray-700 border-t-blue-500"></div>
            </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {ident.is_primary && (
                      <span className="px-1.5 py-0.5 bg-blue-500/20 text-blue-400 text-[10px] font-bold rounded border border-blue-500/20">PRIMARY</span>
                    )}
                  </div>
                  <p className="text-xs font-mono text-blue-300 truncate mt-0.5">{ident.provider_user_id}</p>
                  {ident.scw_address && (
      ...
      <div className="bg-[#1e293b]/70 border border-gray-700/50 shadow-2xl backdrop-blur-md rounded-2xl overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-700/50 flex items-center justify-between">
          ...
          <button
            onClick={copyToken}
            className={`text-xs font-medium transition-colors ${tokenCopied ? 'text-green-400' : 'text-blue-400 hover:text-blue-300'}`}
          >
        ...
        {showToken && (
          <div className="p-4">
            <pre className="text-xs font-mono text-blue-300/80 break-all whitespace-pre-wrap bg-[#0b1120] rounded-lg p-4 max-h-32 overflow-y-auto border border-gray-700/50">
              {token}
            </pre>
          </div>
        )}
  ...
  if (!userInfo) {
    return (
      <div className="h-screen w-full bg-[#0b1120] flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-4 border-gray-800 border-t-blue-500"></div>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-[#0b1120] text-gray-300 font-sans flex flex-col md:flex-row overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="hidden md:flex w-72 flex-col bg-[#0f172a] border-r border-gray-800 h-screen flex-shrink-0 z-10 shadow-xl">
        ...
        <div className="p-6">
           <div className="flex items-center gap-3 mb-6">
             <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-fuchsia-600 flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/20">W</div>
      ...
      <div className="flex-1 overflow-y-auto h-screen relative">
        ...
        <div className="absolute top-0 inset-x-0 h-[400px] bg-gradient-to-b from-blue-900/10 to-transparent pointer-events-none"></div>
            ...
            <>
              ...
              <header className="mb-10">
                <h2 className="text-3xl md:text-5xl font-extrabold text-white tracking-tight mb-4 flex items-center gap-4">
                  ...
                  <span className="px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm rounded-md font-mono mt-1">{activeToken}</span>
                </h2>
                <p className="text-gray-400 text-base md:text-lg max-w-2xl leading-relaxed">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full px-5 py-3.5 bg-[#0b1120] border border-gray-700 rounded-xl text-white placeholder-gray-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all shadow-inner"
                          disabled={txLoading}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <>
                          <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1 flex items-center justify-between">
                              ...
                              <span className="text-[9px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30">CUSTOM</span>
                            </label>
                            <input
                              type="text"
                    ...
                    {activeToken === 'ERC20' && (
                      <div className="space-y-2 mt-2">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1 flex items-center gap-2">
                          Token Icon
                          <span className="text-[9px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30 normal-case font-normal">Optional</span>
                        </label>
                        ...
                        {!uploadPreview ? (
                          <div onClick={() => fileInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); e.currentTarget.classList.add('border-blue-500') }} onDragLeave={(e) => { e.currentTarget.classList.remove('border-blue-500') }} onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove('border-blue-500'); e.dataTransfer.files?.[0] && handleFileSelect(e.dataTransfer.files[0]) }} className="w-full px-5 py-6 bg-[#0b1120] border-2 border-dashed border-gray-700 rounded-xl text-gray-500 hover:border-gray-500 hover:text-gray-400 transition-all cursor-pointer flex flex-col items-center gap-2">
                          ...
                          <div className="flex items-center gap-4 p-3 bg-[#0b1120] border border-gray-700 rounded-xl">
                            ...
                            {uploadProgress === 'uploading' && <svg className="animate-spin h-5 w-5 text-blue-400" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>}
                            {uploadProgress === 'done' && <span className="text-blue-400 text-lg">✓</span>}
                        ...
                        {portfolioLoading ? (
                          ...
                          <div className="relative">
                            <select
                              ...
                              className="w-full px-5 py-3.5 bg-[#0b1120] border border-gray-700 rounded-xl text-white outline-none transition-all shadow-inner focus:border-blue-500 appearance-none pr-10"
                              disabled={txLoading}
                            >
                      ...
                      {(!interactTokenAddress || !portfolio.filter(p => p.type.replace('-', '') === activeToken && (activeSection === 'mint' ? p.isOwner : true)).some(p => p.address === interactTokenAddress)) && (
                        <div className="space-y-2 animate-fade-in">
                          ...
                          <input
                            ...
                            className="w-full px-5 py-3.5 font-mono text-sm bg-[#0b1120] border border-gray-700 rounded-xl text-blue-300 placeholder-gray-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all shadow-inner"
                            disabled={txLoading}
                          />
                        </div>
                      )}
                        ...
                        <div className="space-y-2 relative">
                          ...
                          <div className="relative">
                            <input
                              ...
                              className={`w-full px-5 py-3.5 bg-[#0b1120] border ${amountError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-700 focus:border-blue-500 focus:ring-blue-500'} rounded-xl text-white placeholder-gray-700 focus:ring-1 outline-none transition-all shadow-inner ${activeSection === 'transfer' && selectedAsset ? 'pr-20' : ''}`}
                            ...
                            {activeSection === 'transfer' && selectedAsset && (
                              <button
                                ...
                                className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-1 bg-blue-500/10 text-blue-400 hover:text-blue-300 hover:bg-blue-500/20 text-xs font-bold rounded-lg border border-blue-500/20 transition-colors shadow-sm"
                                disabled={txLoading}
                              >
                                MAX
                              </button>
                      ...
                      {activeToken !== 'ERC20' && (
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full px-5 py-3.5 bg-[#0b1120] border border-gray-700 rounded-xl text-white placeholder-gray-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all shadow-inner disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            <div className="flex items-start gap-4 p-4 bg-[#0b1120] border border-gray-700 rounded-xl">
                              ...
                              <div className="flex-1 min-w-0 space-y-1">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  {uploadProgress === 'done' && <span className="text-xs text-blue-400">✓ Uploaded to MinIO</span>}
                    ...
                    className={`w-full py-4 px-6 rounded-xl font-bold text-white shadow-lg transition-all border border-black/10 flex items-center justify-center
                      ${(!smartWalletAddress || (activeSection === 'deploy' ? (!deployName || !deploySymbol) : (!interactTokenAddress || interactTokenAddress === 'custom' || interactTo === 'custom')))
                        ...
                        : txLoading
                          ...
                          : 'bg-gradient-to-r from-blue-600 to-fuchsia-600 hover:from-blue-500 hover:to-fuchsia-500 transform hover:-translate-y-0.5 hover:shadow-blue-500/25'}
                    `}
                  >
                    {txLoading ? (
              ...
              {txResult && (
                <div className="bg-[#0b1120]/80 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-blue-400 to-fuchsia-500"></div>

                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3 text-blue-400 font-bold tracking-wide">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div className="bg-[#1e293b]/50 border border-gray-700/50 rounded-xl p-4" onContextMenu={(e) => handleContextMenu(e, txResult.transaction_hash, 'tx')}>
                      ...
                      <div className="flex items-center gap-3">
                        <p className="flex-1 text-sm font-mono text-blue-300 truncate">{txResult.transaction_hash}</p>
                        <button
                          onClick={() => {
                          ...
                          className={`flex-shrink-0 p-2 rounded-lg shadow-sm transition-all border ${
                            txCopied
                              ...
                              : 'text-gray-400 hover:text-white bg-[#0b1120] border-gray-700 hover:border-blue-500/50'
                          }`}
            ...
            >
              ...
              {getContextMenuItems(contextMenu.type).map((item, i) => (
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:bg-blue-600/20 hover:text-white transition-colors flex items-center gap-2"
                >
                  {item.label}
                </button>
              ))}
