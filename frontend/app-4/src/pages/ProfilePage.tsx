...
const CopyableField = ({ label, value, isMono = false }: { label: string, value: string | boolean, isMono?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs font-medium transition-opacity ${copied ? 'text-green-600 opacity-100' : 'text-violet-600 opacity-0 group-hover:opacity-100'}`}>
        {copied ? 'Copied!' : 'Copy'}
      </span>
    </div>
  )
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <div className="animate-spin rounded-full h-10 w-10 border-4 border-violet-200 border-t-violet-600 mb-4"></div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-xl p-6 border shadow-sm ${
        isWalletUser
          ? 'bg-gradient-to-br from-pink-50 to-fuchsia-50 border-pink-100'
        ...
        <div className="flex items-center gap-4">
          {userInfo?.picture ? (
            ...
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-pink-500 to-fuchsia-600 flex items-center justify-center text-white text-2xl font-bold shadow-md">
              ⟠
            </div>
          ) : (
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-600 flex items-center justify-center text-white text-xl font-bold shadow-md">
          ...
          <div className="min-w-0 flex-1">
            {isWalletUser ? (
              <>
                ...
                <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-xs font-medium bg-pink-100 text-pink-800">
                  ⟠ SIWE Connected
                </span>
              </>
            ) : (
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
          ...
          <button
            onClick={copyToken}
            className={`text-xs font-medium transition-colors ${tokenCopied ? 'text-green-600' : 'text-violet-600 hover:text-violet-800'}`}
          >
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          ...
          className="flex-1 bg-violet-50 hover:bg-violet-100 text-violet-700 font-medium py-2.5 rounded-lg transition-colors text-sm"
        >
          Dashboard
        </button>
      </div>
