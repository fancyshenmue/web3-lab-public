                    ...
                    {activeToken === 'ERC20' && (
                      <div className="space-y-2 mt-2">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1 flex items-center gap-2">
                          Token Icon
                          <span className="text-[9px] bg-violet-500/20 text-violet-400 px-2 py-0.5 rounded border border-violet-500/30 normal-case font-normal">Optional</span>
                        </label>
                        ...
                        {!uploadPreview ? (
                          <div onClick={() => fileInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); e.currentTarget.classList.add('border-violet-500') }} onDragLeave={(e) => { e.currentTarget.classList.remove('border-violet-500') }} onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove('border-violet-500'); e.dataTransfer.files?.[0] && handleFileSelect(e.dataTransfer.files[0]) }} className="w-full px-5 py-6 bg-[#0b1120] border-2 border-dashed border-gray-700 rounded-xl text-gray-500 hover:border-gray-500 hover:text-gray-400 transition-all cursor-pointer flex flex-col items-center gap-2">
                          ...
                          <div className="flex items-center gap-4 p-3 bg-[#0b1120] border border-gray-700 rounded-xl">
                            ...
                            {uploadProgress === 'uploading' && <svg className="animate-spin h-5 w-5 text-violet-400" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>}
                            {uploadProgress === 'done' && <span className="text-violet-400 text-lg">✓</span>}
                            ...
                            <div className="flex items-start gap-4 p-4 bg-[#0b1120] border border-gray-700 rounded-xl">
                              ...
                              <div className="flex-1 min-w-0 space-y-1">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  {uploadProgress === 'done' && <span className="text-xs text-violet-400">✓ Uploaded to MinIO</span>}
