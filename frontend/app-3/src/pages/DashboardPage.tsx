            ...
            {(['ERC20', 'ERC721', 'ERC1155'] as TokenType[]).map(t => (
              <button
                ...
                className={`w-full text-left pl-14 pr-4 py-2 text-[13px] transition-all
                  ${activeToken === t
                    ? 'text-white font-medium bg-amber-600/20 shadow-[inset_2px_0_0_0_#3b82f6]'
        ...
        <div className="p-4 space-y-3">
          {identitiesLoading ? (
            <div className="text-center py-6">
              <div className="animate-spin inline-block rounded-full h-6 w-6 border-2 border-gray-700 border-t-amber-500"></div>
            </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {ident.is_primary && (
                      <span className="px-1.5 py-0.5 bg-amber-500/20 text-amber-400 text-[10px] font-bold rounded border border-amber-500/20">PRIMARY</span>
                    )}
                  </div>
                  <p className="text-xs font-mono text-amber-300 truncate mt-0.5">{ident.provider_user_id}</p>
                  {ident.scw_address && (
                        ...
                        {portfolioLoading ? (
                          ...
                          <div className="relative">
                            <select
                              ...
                              className="w-full px-5 py-3.5 bg-[#0b1120] border border-gray-700 rounded-xl text-white outline-none transition-all shadow-inner focus:border-amber-500 appearance-none pr-10"
                              disabled={txLoading}
                            >
                        ...
                        <div className="space-y-2 relative">
                          ...
                          <div className="relative">
                            <input
                              ...
                              className={`w-full px-5 py-3.5 bg-[#0b1120] border ${amountError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-700 focus:border-amber-500 focus:ring-amber-500'} rounded-xl text-white placeholder-gray-700 focus:ring-1 outline-none transition-all shadow-inner ${activeSection === 'transfer' && selectedAsset ? 'pr-20' : ''}`}
                            ...
                            {activeSection === 'transfer' && selectedAsset && (
                              <button
                                ...
                                className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-1 bg-amber-500/10 text-amber-400 hover:text-amber-300 hover:bg-amber-500/20 text-xs font-bold rounded-lg border border-amber-500/20 transition-colors shadow-sm"
                                disabled={txLoading}
                              >
                                MAX
                              </button>
