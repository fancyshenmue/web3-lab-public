...
const CopyableField = ({ label, value, isMono = false }: { label: string, value: string | boolean, isMono?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs font-medium transition-opacity ${copied ? 'text-green-600 opacity-100' : 'text-emerald-600 opacity-0 group-hover:opacity-100'}`}>
        {copied ? 'Copied!' : 'Copy'}
      </span>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className={`rounded-xl p-6 border shadow-sm ${
        ...
        <div className="flex items-center gap-4">
          {userInfo?.picture ? (
            ...
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center text-white text-2xl font-bold shadow-md">
              ⟠
            </div>
          ) : (
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
          ...
          <button
            onClick={copyToken}
            className={`text-xs font-medium transition-colors ${tokenCopied ? 'text-green-600' : 'text-emerald-600 hover:text-emerald-800'}`}
          >
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          ...
          className="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-medium py-2.5 rounded-lg transition-colors text-sm"
        >
          Dashboard
        </button>
      </div>
