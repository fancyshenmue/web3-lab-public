...
- [x] 7.1 Add `deployedContractAddress` and `contractAddrCopied` state variables to track and display the newly deployed contract address.
- [x] 7.2 Parse transaction receipt logs for factory events (`ERC20Created`, `ERC721Created`, `ERC1155Created`) to extract the deployed contract address from `topics[1]`.
- [x] 7.3 Add "Deployed Contract Address" UI block in the transaction result terminal with copy-to-clipboard and right-click context menu (Blockscout explorer, holdings, etc.).
- [x] 7.4 Fix `executeTransaction` to only reset `deployedContractAddress` on `deploy_contract` actions, not on secondary `set_uri` calls.
- [x] 7.5 Refactor `set_uri` from `await executeTransaction('set_uri')` to a fire-and-forget `fetch()` call, preventing the secondary transaction from overwriting the deploy TX hash and contract address in the UI.
- [x] 7.6 Propagate all changes consistently across all four frontend applications (`app`, `app-2`, `app-3`, `app-4`).
